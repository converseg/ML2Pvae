...
## Model Training
For users unfamiliar with Tensorflow and Keras, this package provides a simple wrapper function which allows for training of an ML2P-VAE model. TODO: more here
...
After a model has been trained, the item parameter estimates are found in the trainable variables in the VAE. TODO: more here
...
The encoder of a trained ML2P-VAE model maps student responses to their estimates of their ability parameter. TODO: more here
...
```{r}
# Load data
data <- as.matrix(ML2Pvae::responses)
Q <- as.matrix(ML2Pvae::q_matrix)
cov <- as.matrix(ML2Pvae::correlation_matrix)
...
history_cor <- train_model(vae_cor, data_train, num_epochs = num_epochs, batch_size = batch_size, verbose = 0)
plot(history_cor)
```
```{r}
...
```{r}
# Load in true values
disc_true <- as.matrix(ML2Pvae::disc_true)
diff_true <- as.matrix(ML2Pvae::diff_true)
theta_true<- as.matrix(ML2Pvae::theta_true)
```
