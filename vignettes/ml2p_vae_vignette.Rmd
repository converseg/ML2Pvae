...
# Examples
In this section, an example script is given showing how to construct, train, and evaluate an ML2P-VAE model. Three ML2P-VAE models are constructed, and all use the same data described previously.
...
plot(history_ind, main = 'Model 1 Training Loss',
     sub = '',
     xlab = 'Epoch',
     ylab = 'Loss')
```
...
plot(history_cor, main = 'Model 2 Training Loss',
     sub = '',
     xlab = 'Epoch',
     ylab = 'Loss')
```
...
plot(history_1pl, main = 'Model 3 Training Loss',
     sub = ' ',
     xlab = 'Epoch',
     ylab = 'Loss')
```
...
matplot(t(disc_true), t(disc_est_cor), pch = '*',
        xlim = c(0.05, 2), ylim = c(0.05, 2),
        main = 'Discrimination Parameters',
        sub = 'Assuming knwon correlation between latent traits',
        xlab = 'True', ylab = 'Estimated')
par(pty="s")
