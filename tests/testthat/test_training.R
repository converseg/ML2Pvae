...
test_that("full covariance vae can be fit to data", {
  ...
  decoder <- models[[2]]
  vae <- models[[3]]
  fit_full_cov_model(encoder, decoder, vae, mean_vector, cov, train_data, num_epochs = 3, verbose = 0)
  encoded_test <- encoder(test_data)
  enc_test_mean <- encoded_test[[1]]
...
test_that("single-dimensional IRT estimation works", {
  ...
  models <- build_vae_standard_normal(3, 1, Q, model_type = 1, enc_hid_arch = c(4,2))
  encoder <- models[[1]]
  decoder <- models[[2]]
  vae <- models[[3]]
<<<<<<< HEAD
